...
def main(
    ...
    try:
        ...
        monthData = radarbymonth(
            radarCodes[radarName], time.strftime(directory))
        with open('tmp.pkl', 'wb') as f:
            pickle.dump(monthData, f)

    print(len(monthData[0]))
    histogram(monthData[0], monthData[1], monthData[2],
              "%s: %s" % (radarName, time.strftime('%Y/%m')))
...
def radarbymonth(radar_code, directory):
    ...
    for entry in os.scandir(directory):
        if radar_code in str(entry):
            ...
            if dataLen > 0:
                outlier = filter_radar_data.flag_interference(
                    copy.deepcopy(unfiltered))
                scatter = filter_radar_data.scatter_filter(
                    copy.deepcopy(outlier))
                unfilteredMonth += list(unfiltered["vel"]
                                        [unfiltered["gs"] == 0])
