...
NETCDF_DIR_FMT = helper.FIT_NC_DIR_FMT
...
def main(date):
    ...
    if DOWNLOAD_SOURCE_FILES:
        ...
        subprocess.call(
            'scp -r \'radar@wallops.invalid:/borealis_nfs/borealis_data/rawacf_dmap/202212*\' /project/superdarn/data/rawacf/2022/12', shell=True)

    convert_rawacf_to_fitacf_and_netcdf(
        startDate, endDate, rawDir, fitDir, netDir)
    ...
    emailBody = '"Finished downloading and converting {month} RawACF data\nTotal time: {time}"'.format(
        month=startDate.strftime('%Y/%m'), time=totalTime)
...
def download_files_from_globus(rawDir, netDir, date):
    ...
    subprocess.call(
        '{0} -start -restrict-paths \'rw~/,rw/project/superdarn/data\' &'.format(helper.GLOBUS_PATH), shell=True)

    # Initiate the transfer from Globus to APL
    subprocess.call('nohup /project/superdarn/software/python-3.8.1/bin/python3 /homes/superdarn/superdarn/globus/sync_radar_data_globus.py -y {0} -m {1} -t {2} {3}'.format(
        date.year, date.month, pattern, fitDir), shell=True)

    # Stop Globus Connect Personal
    subprocess.call('{0} -stop'.format(helper.GLOBUS_PATH), shell=True)

    emailSubject = '"{0} rawACF Data Successfully Downloaded"'.format(
        date.strftime('%Y/%m'))
    emailBody = '"{0} rawACF source files have been downloaded. Starting conversion to fitACF and netCDF."'.format(
        date.strftime('%Y/%m'))
    helper.send_email(emailSubject, emailBody)
...
def download_files_from_bas(rawDir, netDir, startDate):
    ...
    rsyncLogDir = LOG_DIR + \
        'BAS_rsync_logs/{yr}'.format(yr=startDate.strftime('%Y'))
    os.makedirs(rsyncLogDir, exist_ok=True)
    rsyncLogFilename = '{dir}/BAS_rsync_{month}.out'.format(
        dir=rsyncLogDir, month=fileNameDateString)

    # Save a list of all rawACF files on BAS for the given month and store it in the netcdf directory
    os.system('ssh apl@{bas} ls /sddata/raw/{date} > {ncDir}bas_rawacfs_{dateSuffix}.txt'.format(
        bas=BAS_SERVER, date=dateString, ncDir=netDir, dateSuffix=fileNameDateString))
    # Remove the first line (hashes filename)
    os.system('sed -i \'1d\' {ncDir}bas_rawacfs_{dateSuffix}.txt'.format(
        ncDir=netDir, dateSuffix=fileNameDateString))

    numTries = 0
    rsyncSuccess = False
    while numTries < MAX_NUM_RSYNC_TRIES:
        os.system('nohup rsync -rv apl@{bas}:{basRaw} {aplRaw} >& {logFile}'.format(
            bas=BAS_SERVER, basRaw=basRawDir, aplRaw=rawDir, logFile=rsyncLogFilename))

        # Check that all files were copied
        os.system('ls {aplRaw} > {ncDir}bas_rawacfs_copied_{dateSuffix}.txt'.format(
            aplRaw=rawDir, ncDir=netDir, dateSuffix=fileNameDateString))

        basList = "{ncDir}bas_rawacfs_{dateSuffix}.txt".format(
            ncDir=netDir, dateSuffix=fileNameDateString)
        aplList = "{ncDir}bas_rawacfs_copied_{dateSuffix}.txt".format(
            ncDir=netDir, dateSuffix=fileNameDateString)
    ...
    if not rsyncSuccess:
        ...
        emailSubject = '"Unsuccessful attempt to copy {date} BAS rawACF Data"'.format(
            date=dateString)
        emailBody = '"Tried to copy {date} rawACFs from BAS {num} times, but did not succeed. \nSee {logfile} for more details."'.format(
            date=dateString, num=MAX_NUM_RSYNC_TRIES, logfile=rsyncLogFilename)
        helper.send_email(emailSubject, emailBody)
        sys.exit('{message}'.format(message=emailBody))

    # Send a notification email saying the rsync succeeded
    emailSubject = '"{date} BAS rawACF Data Successfully Downloaded"'.format(
        date=dateString)
    emailBody = '"{date} rawACF files from BAS have been downloaded. Starting conversion to fitACF and netCDF."'.format(
        date=dateString)
    os.system('rm {ncDir}bas_rawacfs_copied_{dateSuffix}.txt'.format(
        ncDir=netDir, dateSuffix=fileNameDateString))
    helper.send_email(emailSubject, emailBody)
...
def convert_rawacf_to_fitacf_and_netcdf(startDate, endDate, rawDir, fitDir, netDir):
    ...
    emailSubject = '"{date} rawACF to netCDF Conversion Successful"'.format(
        date=dateString)
    emailBody = 'Finished converting {date} rawACF files to fitACF and netCDF'.format(
        date=dateString)
    helper.send_email(emailSubject, emailBody)
...
def get_first_and_last_days_of_month(date):
    ...
    lastDayOfMonth = (firstDayOfMonth + relativedelta(months=1)
