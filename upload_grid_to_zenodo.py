...
def upload_to_zenodo(sandbox, date):
    ...
    uploadDir = date.strftime(helper.GRID_DIR_FMT)
    ...
    if date.year > helper.LATEST_PUBLIC_DATA:
        data = {
            'metadata': {
                'title': 'Wallops SuperDARN Grid data in netCDF format ({0})'.format(date.strftime('%Y-%b')),
                'upload_type': 'dataset',
        ...
        data = {
            'metadata': {
                'title': 'SuperDARN Grid data in netCDF format ({0})'.format(date.strftime('%Y-%b')),
                'upload_type': 'dataset',
