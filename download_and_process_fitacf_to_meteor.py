...
def main(date, two_five, three_zero):
    ...
    if two_five:
        ...
        convert_fitacf_to_meteor_wind_and_gridnc(
            startDate, endDate, fitDir, meteorWindDir, 2.5)
#        remove_converted_files(fitDir)
    ...
    if three_zero:
        ...
        convert_fitacf_to_meteor_wind_and_gridnc(
            startDate, endDate, fitDir, meteorWindDir, 3.0)
        # remove_converted_files(fitDir)
        # os.rmdir(fitDir)
...
def convert_fitacf_to_meteor_wind_and_gridnc(startDate, endDate, fitDir, windDir, fitVersion):
   ...
   # fit_to_grid_nc.main(startDate, endDate)
    ...
    emailSubject = '"{date} fitACF to meteor wind Conversion Successful"'.format(
        date=dateString)
    emailBody = 'Finished converting {date} fitACF files to meteor wind'.format(
        date=dateString)
    helper.send_email(emailSubject, emailBody)
