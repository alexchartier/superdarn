...
def main(
        starttime=dt.datetime(2019, 1, 1),
        endtime=dt.datetime(2019, 2, 1),
        fit_fname_fmt='/project/superdarn/data/fitacf/%Y/%m/%Y%m%d',
        #wind_fname_fmt='/project/superdarn/data/meteorwind/%Y/%m/%Y%b%d',
        wind_fname_fmt='/project/superdarn/data/meteorwind_test/%Y/%m/%Y%m%d',
        run_dir='./run_mw/',
    ...
    while time <= endtime:
        for radar_name, hdw_params in radar_list.items():
            ...
            fit_fname_regex = time.strftime(
                fit_fname_fmt) + '*.{0}.*fitacf3'.format(radar_name)
...
if __name__ == '__main__':
    ...
    assert len(args) == 5, 'Should have 4x args, e.g.:\n' + \
        'python3 fit_to_meteorwind.py 20160101 20170101 ' + \
