function ctmt = calc_ctmt_wind(model_coeffs, hours, lons)
...
% lons = 0:15:360;
% 
% %% Save coefficients as .mat for speed
% try
...
% end
%
% ctmt = calc_ctmt_wind(model_coeffs, hours, lons)
% 
% savestruct(out_fn, ctmt)
% 
% for hi = 1:25
%     contourf(lons, lats, squeeze(ctmt.wind_ut(1, hi, :, :, alts == 100, months==9))')
%     colorbar
%     pause(0.5)
% end
% contourf(lons, lats, squeeze(ctmt.wind_lst(1, hours==0, :, :, alts==100, months==9))')
...
ctmt.alts = double(alts); 
ctmt.lats = double(lats); 
ctmt.lons = double(lons); 
