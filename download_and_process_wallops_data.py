...
def main(date):
    ...
    subprocess.call('scp -r \'radar@wallops.invalid:/borealis_nfs/borealis_data/rawacf_dmap/{year}{month}*\' /project/superdarn/data/rawacf/{year}/{month}'.format(
        year=startDate.strftime('%Y'), month=startDate.strftime('%m')), shell=True)
...
def convert_rawacf_to_fitacf_and_netcdf(startDate, endDate, rawDir, fitDir, netDir):
    ...
    fit_to_meteorwind.main(
        startDate, endDate, fitFilenameFormat, windFilenameFormat)
