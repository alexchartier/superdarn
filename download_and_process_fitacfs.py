...
def download_fitacfs_from_globus(fitDir, date, pattern):
    ...
    subprocess.call(
        '{0} -start -restrict-paths \'rw~/,rw/project/superdarn/data/fitacf\' &'.format(helper.GLOBUS_PATH), shell=True)

    # Initiate the Globus -> APL transfer
    subprocess.call('nohup /software/python-3.11.4/bin/python3 /homes/superdarn/superdarn/globus/sync_radar_data_globus.py -y {0} -m {1} -t {2} {3}'.format(
    ...
    emailSubject = '"{0} {1} Data Successfully Downloaded"'.format(
        date.strftime('%Y/%m'), pattern)
    emailBody = '"{0} {1} source files have been downloaded. Starting conversion to netCDF."'.format(
        date.strftime('%Y/%m'), pattern)
    helper.send_email(emailSubject, emailBody)
...
def convert_fitacf_to_netcdf(startDate, endDate, fitDir, netDir, fitVersion):
    ...
    emailSubject = '"{date} fitACF to netCDF Conversion Successful"'.format(
        date=dateString)
    emailBody = 'Finished converting {date} fitACF files to netCDF'.format(
        date=dateString)
    helper.send_email(emailSubject, emailBody)
